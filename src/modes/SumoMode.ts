...
export class SumoMode extends GameMode {
  ...
  public update(delta: number): void {
    ...
    this.zoneX =
      this.arenaCenterX + Math.cos(this.elapsed * this.ZONE_ORBIT_SPEED) * currentOrbitRadius;
    this.zoneZ =
      this.arenaCenterZ + Math.sin(this.elapsed * this.ZONE_ORBIT_SPEED) * currentOrbitRadius;
  ...
  private handlePlayerCollisionPush(): void {
    ...
    if (dist < this.PUSH_RADIUS && dist > 0.01) {
      ...
      if (relDot > 0) {
        ...
        this.p1Body.applyImpulse(new CANNON.Vec3(-pushImpulse.x, 0, -pushImpulse.z));
      }
    }
  }
