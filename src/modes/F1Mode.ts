...
export class F1Mode extends GameMode {
  ...
  public getLapsP1(): number {
    return this.stateP1.laps;
  }
  public getLapsP2(): number {
    return this.stateP2.laps;
  }

  public getP1PowerInfo(): string {
    return this.getActivePowerInfo(this.stateP1);
  }
  public getP2PowerInfo(): string {
    return this.getActivePowerInfo(this.stateP2);
  }
  ...
  private getActivePowerInfo(state: CarState): string {
    ...
    if (state.mirrorTimer > 0) {
      effects.push(`🪞 ${Math.ceil(state.mirrorTimer)}s`);
    }
    if (state.turboTimer > 0) {
      effects.push(`⚡ ${Math.ceil(state.turboTimer)}s`);
    }
    if (state.obstacleSlowTimer > 0) {
      effects.push(`🛑 ${Math.ceil(state.obstacleSlowTimer)}s`);
    }
    return effects.join(' ');
  }
  ...
  private createTrackSurface(): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      vertices.push(
        p.x + (nx / len) * TRACK_HALF_WIDTH,
        TRACK_Y,
        p.z + (nz / len) * TRACK_HALF_WIDTH,
        p.x - (nx / len) * TRACK_HALF_WIDTH,
        TRACK_Y,
        p.z - (nz / len) * TRACK_HALF_WIDTH
      );
  ...
  private createKerbRibbon(offset: number): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      vertices.push(
        p.x + ux * (offset - width / 2),
        TRACK_Y + 0.006,
        p.z + uz * (offset - width / 2),
        p.x + ux * (offset + width / 2),
        TRACK_Y + 0.006,
        p.z + uz * (offset + width / 2)
      );
  ...
  private updateCar(state: CarState, controls: PlayerControls, delta: number): void {
    const input = this.engine.input;

    let effectiveMax = MAX_SPEED;
    if (state.penaltyTimer > 0) {
      effectiveMax *= PENALTY_SPEED_FACTOR;
    }
    if (state.turboTimer > 0) {
      effectiveMax *= TURBO_SPEED_MULT;
    }
    if (state.obstacleSlowTimer > 0) {
      effectiveMax *= OBSTACLE_SLOW_FACTOR;
    }
  ...
  private updateTimers(state: CarState, delta: number): void {
    ...
    if (state.mirrorTimer > 0) {
      state.mirrorTimer = Math.max(0, state.mirrorTimer - delta);
    }
    if (state.turboTimer > 0) {
      state.turboTimer = Math.max(0, state.turboTimer - delta);
    }
    if (state.obstacleSlowTimer > 0) {
      state.obstacleSlowTimer = Math.max(0, state.obstacleSlowTimer - delta);
    }
  }
